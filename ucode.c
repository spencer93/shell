...
int parseLine(char * line, char **values, char delimiter){
    char *lp = line;
    int index = 0, numTokens = 0;
    if (strlen(line) == 0)
      return 0;
    numTokens++;
    values[index++] = lp;
...
// if finished is set to 0 we are done, we hit the end of the file, no guarantee buf ends with null terminator
// if finished is 1 then buf ends with a null terminator
int fileGetLine(int fd, char buf[],int * finished){
  char mc[10];
  int index = 0;
  int complete = 1;
  while(index < 1024){ buf[index++] = '\0'; } // guaran
  index = 0;
  while((complete = read(fd,mc,1)) && index < 1024){
    if(mc[0] == '\n')
        break;
    else
        buf[index++] = mc[0];

    if(index == 1023){ // dont read anymore characters
      *finished = 1; // more to read as far as can be discerned
      return -1; // line was too long, filled whole buffer before finding new line, still ends with null terminator though
    }
    mc[0] = '\0';
  }
  *finished = complete;
  return index;// return lines length including newline char
}

int inputRedirected(){
  struct stat ttystatbuf;
  struct stat stdinstatbuf;
  char tty[256];

  gettty(tty);
  stat(tty,&ttystatbuf);
  fstat(0,(char*)&stdinstatbuf);
/*
  printf("tty.dev: %d\n\r",ttystatbuf.st_dev);
  printf("tty.ino: %d\n\r",ttystatbuf.st_ino);
  printf("0.dev: %d\n\r",stdinstatbuf.st_dev);
  printf("0.ino: %d\n\r",stdinstatbuf.st_ino);
*/

  if(ttystatbuf.st_dev == stdinstatbuf.st_dev && ttystatbuf.st_ino == stdinstatbuf.st_ino)
    return 0; // input not redirected
  else
    return 1; // input has been redirected
}
int outputRedirected(){
  struct stat ttystatbuf;
  struct stat stdoutstatbuf;
  char tty[256];

  gettty(tty);
  stat(tty,&ttystatbuf);
  fstat(1,(char*)&stdoutstatbuf);

/*
  printf("tty.dev: %d\n\r",ttystatbuf.st_dev);
  printf("tty.ino: %d\n\r",ttystatbuf.st_ino);
  printf("1.dev: %d\n\r",stdoutstatbuf.st_dev);
  printf("1.ino: %d\n\r",stdoutstatbuf.st_ino);
*/
  if(ttystatbuf.st_dev == stdoutstatbuf.st_dev && ttystatbuf.st_ino == stdoutstatbuf.st_ino)
    return 0; // output not redirected
  else
